...
void ssa_heap_domaint::transform(const namespacet &ns, domain_baset::locationt from,
  ...
  else if (from->is_function_call() && from->function == to->function)
  {
    ...
    if (function_map.find(fun_id) != function_map.end())
    {
      ...
      for (auto &obj : function_map.at(fun_id).new_objects)
      {
        ...
        {
          function_map[function].new_objects.insert(std::make_pair(new_obj, std::set<exprt>()));
        }

        for (auto &expr : obj.second)
        {
          const exprt pointer = function_map.at(fun_id).corresponding_expr(expr,
                                                                           fun_call.arguments(), 0);
          ...
          for (auto &o : old_objects)
          {
            if (o.id() == ID_symbol && o.type().get_bool("#dynamic") &&
                new_obj != o)
              function_map[function].new_objects.at(to_symbol_expr(o)).erase(pointer);
          }
        }
      }
      ...
      {
        const exprt caller_obj = function_map.at(fun_id).corresponding_expr(obj,
                                                                            fun_call.arguments(),
                                                                            0);
...
bool ssa_heap_domaint::merge(const ssa_heap_domaint &other, domain_baset::locationt to)
{
  bool result = false;
  ...
  if (to->function == "" || to->function == other.function)
  {
    ...
    for (auto &other_value : other.value_map)
    {
      ...
      else
      {
        ...
        value_map[other_value.first].insert(other_value.second.begin(), other_value.second.end());
  ...
  for (auto &f : other.function_map)
  {
    ...
    function_map[f.first].modified_objects.insert(f.second.modified_objects.begin(),
...
void ssa_heap_domaint::assign_rhs(const exprt &rhs, const irep_idt &function, objectst &objects,
                                  const namespacet &ns)
{
  if (rhs.get_bool("#malloc_result"))
  {
    ...
    const symbol_exprt new_object = to_symbol_expr(to_address_of_expr(malloc_result).object());

    function_infot &function_info = function_map[function];
    if (function_info.new_objects.find(new_object) == function_info.new_objects.end())
    {
      function_info.new_objects.insert(std::make_pair(new_object, std::set<exprt>()));
    }
...
bool ssa_heap_domaint::is_function_output(const exprt &expr, const irep_idt &function,
                                          const namespacet &ns, bool in_deref)
{
  if (expr.id() == ID_dereference)
  {
    return is_function_output(to_dereference_expr(expr).pointer(), function, ns, true);
  }
  else if (expr.id() == ID_member)
  {
    return is_function_output(to_member_expr(expr).compound(), function, ns, in_deref);
  }
  else if (expr.id() == ID_symbol)
  {
    const symbol_exprt &symbol_expr = to_symbol_expr(expr);
    if (symbol_expr.get_identifier() == id2string(function) + "#return_value")
      return true;
    const symbolt *symbol;
    if (in_deref && !ns.lookup(symbol_expr.get_identifier(), symbol) && symbol->is_parameter)
      return true;
  }
  return false;
}
...
const std::list<symbol_exprt> ssa_heap_domaint::new_objects(const irep_idt &fname) const
{
...
void ssa_heap_domaint::rename_to_caller(symbol_exprt &object, domain_baset::locationt loc,
                                        unsigned &index) const
{
  object.set_identifier("ssa::dynamic_object$" + std::to_string(loc->location_number) + "$" +
                        std::to_string(index++));
}

const std::list<symbol_exprt> ssa_heap_domaint::new_caller_objects(const irep_idt &fname,
...
const std::set<exprt> ssa_heap_domaint::modified_objects(const irep_idt &fname) const
{
...
void ssa_heap_domaint::assign_lhs_rec(const exprt &lhs, const exprt &rhs, const namespacet &ns)
{
...
void ssa_heap_domaint::update_modified(const exprt &expr, const namespacet &ns)
{
  ...
  else if (expr.id() == ID_dereference)
  {
    ...
    if (is_function_output(expr, function, ns, false))
...
void ssa_heap_analysist::initialize(const goto_functionst &goto_functions)
{
  ...
  {
    locationt e = goto_functions.function_map.at("main").body.instructions.begin();
    ...
    forall_goto_functions(f_it, goto_functions)
    {
      if (f_it->second.body_available())
      {
        ...
        {
          entry.function_map[f_it->first].params.push_back(param.get_identifier());
...
void ssa_heap_analysist::init_ptr_param(const exprt &expr, ssa_heap_domaint &f_entry)
{
  ...
  else if (type.id() == ID_struct)
  {
    ...
    {
      if (component.type().id() == ID_pointer && ns.follow(component.type().subtype()) == type)
      {
...
const exprt ssa_heap_domaint::function_infot::corresponding_expr(const exprt &expr,
                                                                 ...
                                                                 unsigned deref_level) const
{
  ...
  {
    return corresponding_expr(to_dereference_expr(expr).pointer(), arguments, deref_level + 1);
  }
  else if (expr.id() == ID_member)
  {
    return corresponding_expr(to_member_expr(expr).compound(), arguments, deref_level);
  }
...
const exprt ssa_heap_domaint::function_infot::apply_deref(const exprt &expr, unsigned level) const
{
  ...
  else if (expr.id() == ID_member && to_member_expr(expr).compound().id() == ID_dereference)
  {
