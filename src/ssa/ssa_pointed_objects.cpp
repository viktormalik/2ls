...
void copy_pointed_info(exprt &dest, const exprt &src, const unsigned max_level)
{
  if(max_level < pointed_level(src))
  {
    ...
    for (unsigned l = 0; l <= max_level; ++l)
    {
      ...
      dest.set(level_str(l, ID_pointer_subtype), src.get(level_str(l, ID_pointer_subtype)));
      if (lvl_pointed_id == ID_symbol)
      {
        dest.set(level_str(l, ID_pointer_sym), src.get(level_str(l, ID_pointer_sym)));
      }
      else
      {
        dest.set(level_str(l, ID_pointer_compound), src.get(level_str(l, ID_pointer_compound)));
        dest.set(level_str(l, ID_pointer_field), src.get(level_str(l, ID_pointer_field)));
      }
    }
  }
}
...
symbol_exprt pointed_object(const exprt &expr, const namespacet &ns)
{
  ...
  symbol_exprt pointed(id2string(ssa_object.get_identifier()) + "'obj", ns.follow(pointed_type));
  ...
  {
    pointed.set(level_str(level, ID_pointer_sym), ssa_object.get_identifier());
  }
  else
  {
    ...
    pointed.set(level_str(level, ID_pointer_compound),
                to_symbol_expr(member.compound()).get_identifier());
    pointed.set(level_str(level, ID_pointer_field), member.get_component_name());
  }

  if (pointed_type.id() == ID_symbol)
  {
    pointed.set(level_str(level, ID_pointer_subtype),
...
const exprt get_pointer(const exprt &expr, unsigned level)
{
  ...
  const typet &pointed_type = symbol_typet(expr.get(level_str(level, ID_pointer_subtype)));

  if (expr.get(level_str(level, ID_pointer_id)) == ID_symbol)
  {
    pointer = symbol_exprt(expr.get(level_str(level, ID_pointer_sym)), pointer_typet(pointed_type));
    copy_pointed_info(pointer, expr, level - 1);
  }
  else
  {
    assert(expr.get(level_str(level, ID_pointer_id)) == ID_member);
    symbol_exprt compound(expr.get(level_str(level, ID_pointer_compound)), expr.type());
    copy_pointed_info(compound, expr, level - 1);
    pointer = member_exprt(compound, pointer_level_field(expr, level), pointer_typet(pointed_type));
  }
  return pointer;
}
...
const symbol_exprt from_dereference(const exprt &expr, const namespacet &ns)
{
  if (expr.id() == ID_symbol)
    return to_symbol_expr(expr);
  else if (expr.id() == ID_dereference)
  {
    const symbol_exprt object = from_dereference(to_dereference_expr(expr).pointer(), ns);
    return pointed_object(object, ns);
  }
  else
    assert(false);
}
...
const irep_idt iterator_to_initial_id(const exprt &expr, const irep_idt &expr_id)
{
  ...
  return id_str.replace(id_str.find(iterator_id_str),
                        iterator_id_str.length(),
                        init_value_id_str);
}
