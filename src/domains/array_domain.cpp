...
void replace_segment_by_index(
  const array_domaint::array_segmentt &segment,
  const exprt &index,
  exprt &expr)
{
  static unsigned i = 0;
  symbol_exprt new_elem_var(
    "elem#tmp#"+std::to_string(i++), segment.elem_var.type());
  ...
  auto new_elem_eq=equal_exprt(
    new_elem_var,
    index_exprt(segment.array_spec.var, index));
  ...
  const exprt &index)
{
  ...
  for(auto &segment : segments)
    result.push_back(
      implies_exprt(
        and_exprt(
          binary_relation_exprt(index, ID_ge, segment->lower_bound),
    ...
    for (auto &dep : segment_deps.dep_sets)
    {
      ...
      else if (dep.id() == ID_index)
      {
           ...
           same_var(dep_index, segment->upper_bound))
        {
          ...
          exprt other_array=SSA.name(
            ssa_objectt(dep_array, ns),
