				      ...
				      bool sufficient)
{
  ...
  if(options.get_bool_option("complexity"))
  {
  ...
  {
    debug() << "function " << 
     (has_function_calls ? "has function calls (BUT THEY ARE IGNORED!)" : 
      " does not have function calls") << eom;
    debug() << "function " << 
      (has_loops ? "has loops" : " does not have loops") << eom;
    if(has_loops) 
    {
      //  template_generator_complexityt template_generator(options,ssa_unwinder.get(function_name));
      //    template_generator(SSA,forward);
      template_generator_summaryt template_generator(options,ssa_unwinder.get(function_name));
      template_generator(SSA,forward);
      ...
      analyzer1.get_result(summary.complexity,template_generator.all_vars());
     
      //statistics
      solver_instances++;
      solver_calls += analyzer1.get_number_of_solver_calls();
    }
    else summary.complexity = true_exprt(); //meaning "constant"
  }
